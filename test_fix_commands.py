...
from src.diagnostics import run_diagnostics, get_fix_commands
...
def test_fix_command_structure():
    ...
    if 'linux' in res['platform'].lower():
        ...
        found = any(token in ' '.join(cmds['audio']) for token in ('apt-get', 'dnf', 'pacman', 'zypper'))
        assert found, f"Expected package manager suggestion in audio commands: {cmds['audio']}"
