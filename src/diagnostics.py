...
def run_diagnostics() -> dict:
    ...
    if not mixer_present:
        ...
        if osname in ("linux", "linux2"):
            ...
            recommendations.append(
                "On other OSes, install SDL_mixer / relevant audio libs or reinstall pygame"
            )

    if not results["pygame_image_extended"]:
        recommendations.append(
            "Image support (PNG/JPEG) appears missing from pygame. Try installing system image dev packages and reinstall Pillow/pygame:"
        )
    ...
    if not svg_support:
        recommendations.append(
            "SVG support (svglib/reportlab) not available — install with: pip install svglib reportlab"
        )

    if not recommendations:
        recommendations.append(
            "No obvious problems detected. If the app still misbehaves, check logs and run the app in verbose mode."
        )
...
def get_fix_commands(results: dict) -> dict:
    ...
    if pkg == "apt":
        ...
        commands["audio"] = ["sudo pacman -S --noconfirm sdl2 sdl2_mixer libsndfile"]
        ...
        commands["audio"] = [
            "# Install SDL2_mixer (your distro package manager) and reinstall pygame"
        ]
...
def interactive_fix_pick(
    results: dict, auto_accept: bool = False, auto_confirm: bool = False
) -> dict:
    ...
    for area, cmd_list in commands.items():
        ...
        if auto_accept:
            ...
            print(
                "Choose which command(s) to run (comma separated numbers), or N to skip:"
            )
        ...
        for cmd in to_run:
            ...
            area_results.append(
                {"command": cmd, "rc": rc, "stdout": out, "stderr": err}
            )
